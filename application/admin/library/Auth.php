...
class Auth extends \fast\Auth
{
    ...
    public function login($username, $password, $keeptime = 0)
    {
		...
		// $times=Db::name('config')->where('name','times')->value('value');
        // if (Config::get('fastadmin.login_failure_retry') && $admin->loginfailure >= $times && time() - $admin->updatetime < 1800) {
        //     $this->setError('请半小时之后再重试');
        //     return false;
        // }
		...
		//登录的时候把group_id插入表中
		$gid=Db::name('auth_group_access')->where('uid',$admin->id)->column('group_id');
		Db::name('admin')->where('id',$admin->id)->setField('group_id',$gid[0]);
    ...
    public function logout()
    {
		...
		//退出的时候把group_id清空
		//Db::name('Admin')->where('id',$this->id)->setField('group_id','');
        $admin->token = '';
        $admin->save();
    ...
    {	
        ...
        // 读取管理员当前拥有的权限节点
        $userRule = $this->getRuleList();//var_dump($userRule);exit;
        $selected = $referer = [];
